...
```json
{
  ...
  "contributes": {
    ...
    "sidebar": [
      { "id": "marketplace", "label": "Store", "icon": "🛒", "route": "/store", "order": 2 }
    ],
    "settings": []
  }
}
...
```json
{
  ...
  "contributes": {
    ...
    "sidebar": [
      { "id": "hello", "label": "Hello", "icon": "👋", "route": "/hello", "order": 100 }
    ],
    "settings": []
  }
}
...
```json
{
  ...
  "contributes": {
    "routes": [
      { "path": "/launch-tracker", "component": "LaunchTrackerPage" }
    ],
    "sidebar": [
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      Object.entries(stats).map(([id, count]) =>
        React.createElement('li', { key: id }, `${id}: ${count} 次`),
      ),
    ),
  );
}
  ...
  activate(ctx: PluginContext) {
    ...
    window.addEventListener('launch-tracker:request', async () => {
      ...
      window.dispatchEvent(
        new CustomEvent('launch-tracker:stats', { detail: stats ?? {} }),
      );
    });
...
*本文档基于 BonNext 插件系统 Phase 5 实现，最后更新：2026-06-20。如需了解架构设计细节，请参阅 `docs/superpowers/specs/2026-06-19-plugin-architecture-design.md`。*
