...
const PROJECT_TTL: Duration = Duration::from_secs(1800);
const POPULAR_TTL: Duration = Duration::from_secs(900);
...
fn evict_expired<T>(map: &mut HashMap<String, CacheEntry<T>>) {
    ...
    if map.len() > MAX_CACHE_SIZE {
        ...
        entries.sort_by_key(|(_, e)| std::cmp::Reverse(e.expires_at));
...
pub struct ApiCache {
    ...
    cf_searches: Mutex<HashMap<String, CacheEntry<String>>>,
    cf_projects: Mutex<HashMap<String, CacheEntry<String>>>,
    cf_featured: Mutex<HashMap<String, CacheEntry<String>>>,
}
...
impl ApiCache {
    pub fn new() -> Self {
        Self {
            ...
            cf_searches: Mutex::new(HashMap::new()),
            cf_projects: Mutex::new(HashMap::new()),
            cf_featured: Mutex::new(HashMap::new()),
        }
    }
    ...
    pub fn cache_search_results(&self, key: &str, results: &(Vec<super::modrinth::ModResult>, u64)) {
        if let Ok(json) = serde_json::to_string(results) {
            Self::put_raw(&self.searches, key.to_string(), json, SEARCH_TTL);
        }
    }

    pub fn get_search_results(&self, key: &str) -> Option<(Vec<super::modrinth::ModResult>, u64)> {
    ...
    pub fn cache_project(&self, slug: &str, project: &super::modrinth::ModProjectFull) {
        if let Ok(json) = serde_json::to_string(project) {
            Self::put_raw(&self.projects, slug.to_string(), json, PROJECT_TTL);
        }
    }
    ...
    pub fn cache_popular(&self, key: &str, results: &[super::modrinth::ModResult]) {
        if let Ok(json) = serde_json::to_string(results) {
            Self::put_raw(&self.popular, key.to_string(), json, POPULAR_TTL);
        }
    }
