    ...
    <div className={styles.container}>
      ...
      <div className={styles.actions}>
        {(status === 'running' || status === 'pending') && (
          <button className={styles.abortBtn} onClick={() => onAbort(workflowId)}>{t('workflow.abort')}</button>
        )}
        {status === 'failed' && (
          <button className={styles.retryBtn} onClick={() => onRetry(workflowId)}>{t('workflow.retry')}</button>
        )}
      </div>
    </div>
  );
