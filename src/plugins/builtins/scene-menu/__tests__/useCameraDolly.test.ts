...
describe('useCameraDolly math', () => {
  it('clampPosition clamps translate to ±0.8 and push to [0, 2.0]', () => {
    expect(clampPosition({ x: 1, y: -1, z: 3 })).toEqual({ x: 0.8, y: -0.8, z: 2.0 });
    expect(clampPosition({ x: -1, y: 1, z: -1 })).toEqual({ x: -0.8, y: 0.8, z: 0 });
